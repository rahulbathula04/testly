...
// Update here → reflects everywhere. Never hard-code prices in UI.
export const EXAM_DATA = {
  GRE:     { id:'GRE',     label:'GRE',     prefix:'ETS', prefixColor:'#1a4fad', labelColor:'#1a4fad', refPrice:26500, testlyPrice:19000, saving:7500  },
  TOEFL:   { id:'TOEFL',   label:'TOEFL',   prefix:'ETS', prefixColor:'#1a4fad', labelColor:'#1a4fad', refPrice:24900, testlyPrice:18500, saving:6400  },
  IELTS:   { id:'IELTS',   label:'IELTS',   prefix:'',    prefixColor:'',        labelColor:'#b91c1c', refPrice:17000, testlyPrice:13000, saving:4000  },
  PTE:     { id:'PTE',     label:'PTE',     prefix:'P',   prefixColor:'#1d4ed8', labelColor:'#1d4ed8', refPrice:18000, testlyPrice:13500, saving:4500  },
  Duolingo:{ id:'Duolingo', label:'Duolingo', prefix:'',   prefixColor:'',        labelColor:'#15803d', refPrice:6300,  testlyPrice:4500,  saving:1800  },
  GMAT:    { id:'GMAT',    label:'GMAT',    prefix:'G',   prefixColor:'#111',    labelColor:'#111',    refPrice:28000, testlyPrice:22000, saving:6000  },
};
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {exams.map((exam) => (
            <button key={exam.id} onClick={() => onBookTest(exam.id)}
              className="bg-white border border-slate-200 rounded-xl p-4 text-left hover:border-slate-400 hover:shadow-md transition-all space-y-3">

              {/* Exam identifier */}
              <div>
                {exam.prefix && (
                  <div className="inline-flex items-center gap-1 border border-slate-200 rounded px-1.5 py-0.5 mb-1">
                    <span className="text-[9px] font-black tracking-wide" style={{ color: exam.prefixColor }}>{exam.prefix}</span>
                  </div>
                )}
                <p className="text-[17px] font-black leading-tight" style={{ color: exam.labelColor }}>
                  {exam.label}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-slate-400 line-through">{fmt(exam.refPrice)}</p>
              </div>

              {/* Testly price */}
              <div>
                <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-400">Testly Price</p>
                <p className="text-[17px] font-black text-slate-900">{fmt(exam.testlyPrice)}</p>
              </div>
